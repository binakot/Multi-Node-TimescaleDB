...
timescaledb-parallel-copy --connection "host=pg_access_node user=postgres password=postgres db=postgres port=5432 sslmode=disable" \
    --table telemetries --columns "imei,time,latitude,longitude,geography,speed,course" --file /tmp/data/data7.csv --copy-options "CSV" \
    --workers 4 --reporting-period 30s
timescaledb-parallel-copy --connection "host=pg_access_node user=postgres password=postgres db=postgres port=5432 sslmode=disable" \
    --table telemetries --columns "imei,time,latitude,longitude,geography,speed,course" --file /tmp/data/data8.csv --copy-options "CSV" \
    --workers 4 --reporting-period 30s
timescaledb-parallel-copy --connection "host=pg_access_node user=postgres password=postgres db=postgres port=5432 sslmode=disable" \
    --table telemetries --columns "imei,time,latitude,longitude,geography,speed,course" --file /tmp/data/data9.csv --copy-options "CSV" \
    --workers 4 --reporting-period 30s
timescaledb-parallel-copy --connection "host=pg_access_node user=postgres password=postgres db=postgres port=5432 sslmode=disable" \
    --table telemetries --columns "imei,time,latitude,longitude,geography,speed,course" --file /tmp/data/data10.csv --copy-options "CSV" \
    --workers 4 --reporting-period 30s
